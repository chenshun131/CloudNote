...
import com.chenshun.utils.orm.entity.BaseEntity;
...
public class HibernateUtils {
    ...
    /**
     * 根据对象ID集合, 整理合并集合. <br/>
     * 页面发送变更后的子对象id列表时,在Hibernate中删除整个原来的子对象集合再根据页面id列表创建一个全新的集合这种看似最简单的做法是不行的. <br/>
     * 因此采用如此的整合算法：在源集合中删除id不在目标集合中的对象,根据目标集合中的id创建对象并添加到源集合中. <br/>
    ...
    public static <T extends BaseEntity> void mergeByCheckedIds(final Collection<T> srcObjects,
                                                                final Collection<String> checkedIds,
                                                                final Class<T> clazz) {
        // 参数校验
